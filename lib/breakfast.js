...
class Breakfast {
  ...
  addBreakfast(date){
    ...
    var breakfast = meal.data.breakfast
    for (var i = 0; i < boxes.length; i++) {
      if (boxes[i].checked) {
        ...
        breakfast.push({name: name, calories: calories});
        breakfastData.push(meal);
        localStorage.setItem('dailyMeals', JSON.stringify(breakfastData));
